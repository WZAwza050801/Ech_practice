...
### 7. Qwen3-Omni —— 候选模型评估（结论：暂不替换主链路，留作实验）

Qwen3-Omni 系列已在硅基流动上线（Instruct / Thinking / Captioner 三个变体），
全模态输入（音频+图像+视频+文本），价格约 **$0.1/M 入、$0.4/M 出**——比现役
Qwen3-VL-32B（$0.29 / $1.0）便宜约 2.5 倍。是否要换？逐段对比：

| 维度 | Qwen3-VL-32B（现役） | Qwen3-Omni |
|---|---|---|
| 输入 | 图像/视频帧 + 文本 | 音频+图像+视频+文本 |
| 价格（入/出，每 M） | $0.29 / $1.0 | **$0.1 / $0.4** |
| 上下文 | **262K** | 66K |
| 音频理解 | 无 | 30 分钟音频、19 种语音 |

分段判断：

- **ASR 段**：faster-whisper 本地免费且够用，换成付费 Omni 纯亏。仅当未来需要
  说话人分离/语义分段这类 whisper 做不好的能力时再考虑。
- **视觉理解段**：Omni 真正的增量是**"边听边看"**——一次调用同时喂某一段的
  音频+关键帧，模型能听见老师口头讲解再配合画面（现役 VL 只能看无声帧，
  纯口头推导、念代码的段落天然丢失信息）。但有两个硬伤：**66K 上下文对 262K
  是实打实降级**（窗口映射模式依赖大上下文，窗口得切更碎）；且接入前必须
  先实测硅基流动 Omni 是否支持帧列表输入格式。
- **写作/规划段**：不相关，纯文本长文写作不是 Omni 主场，维持 Kimi/百炼。

**落地建议**：主链路维持 Qwen3-VL 不动；待统一入口重构后，取一个分 P 做 A/B
实验（VL 链路 vs Omni"音频+帧"链路），对比同一讲义的框架完整度与口头推导
还原度，用真实产物决定是否切换。接入前的 go/no-go 门槛：curl 验证 Omni 的
framelist 输入格式支持。
